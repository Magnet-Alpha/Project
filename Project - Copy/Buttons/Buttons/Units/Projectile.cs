...
namespace Buttons
{
    class Projectile
    {
        ...
        {
            sb.Draw(imgs[0], new Rectangle((int)this.position.X , (int)this.position.Y, 32 * (int)w, 32 * (int)h), new Rectangle(0, 18, 292, 195), Color.White, (float)Math.Atan2(direction.Y, direction.X) - (float)Math.PI/4, new Vector2(0, 0), SpriteEffects.None, 0);
        }
    }
}
